...
def main():
    if len(sys.argv) < 2:
        print("X Usage: python3 test-skeleton.py <image.png>")
        return
    
    img_path = sys.argv[1]
    if not os.path.exists(img_path):
        print(f"X File not found: {img_path}")
        return
    
    print(f"x Loading image: {img_path}")
    orig = Image.open(img_path).conver("RGB")
    w, h = orig.size

    # Convert to numpy for RTMPose
    np_img = np.array(orig)

    print("x Running RTMPose...")
    model = PoseModel("rtmpose-s") # or rtmpose-m
    result = model(np_img)

    keypoints = result["keypoints"][0] # shape: (N, 3)

    print("x Rendering skeleton...")
    ...
    out_path = "images/pose_test.png"
    canvas.save(out_path)

    print(f"x Skeleton saved -> {out_path}")
